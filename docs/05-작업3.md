...
- 진행 현황: 10-1은 6주차 인터뷰 배너만 잔여, 10-2~10-5는 완료. **IRB 신청서는 2026-07-09 제출되어 심사 대기 중**(1차 설문지 포함). 10-6~10-10은 미착수.
- 작업 순서(개정): Research/Engineering 두 트랙을 **병렬** 진행한다. 실험 착수는 두 트랙이 모두 끝나야 가능하다.
  ...
  - **Engineering 트랙**: 10-2 잔여 운영 작업(crontab, 아무 때나) → **10-8(2주 베이스라인 게이트, High — 실험 착수 필수 블로커)** → 10-6(완료 알림, 10-8이 만든 게이트 위에 구현) → 10-7(세밀 다양성 신호, 다른 스토리와 독립적이라 아무 때나 가능) → 10-10(연구 종료 시점 안내, 10-8·10-9 완료 후 착수 — 대조군 일괄 피드백 공개는 실험 착수 전 완료 필요)
...
- **대조군 일괄 피드백 공개**: `VL.weeks` 데이터(주차별 entropy·분포)는 CON도 이미 계산·축적되고 있음 — "종료 전엔 비공개, 종료 후엔 전체 공개"라는 **타이밍 게이트**와, 언락 후 보여줄 화면(기존 주차별 탭 재사용 또는 신규 화면)이 필요.
