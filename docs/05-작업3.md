...
- 진행 현황: 10-1은 6주차 인터뷰 배너만 잔여, 10-2~10-6은 완료 — 10-6은 알림·배지·열람기록 메커니즘 자체는 구현 완료 상태이고, 남은 "베이스라인 종료 후에만 활성" 게이트 연동은 10-8 Task 안에 이미 포함돼 있어 별도 작업이 필요 없다(아래 순서 근거 참고). **IRB 신청서는 2026-07-09 제출되어 심사 대기 중**(1차 설문지 포함). 10-7~10-11은 미착수.
- 작업 순서(개정): Research/Engineering 두 트랙을 **병렬** 진행한다. 실험 착수는 두 트랙이 모두 끝나야 가능하다.
  ...
  - **Engineering 트랙(효율적 순서)**: **10-8(2주 베이스라인 게이트, High — 실험 착수 필수 블로커)** → **10-11(llm.js 피드백 보강, 다른 스토리와 독립적)** → 10-7(세밀 다양성 신호, 10-11의 필터·버전 관리 기반 위에 구현) → 10-10(연구 종료 시점 안내, 10-8·10-9 완료 후 착수 — 대조군 일괄 피드백 공개는 실험 착수 전 완료 필요). 10-2 잔여 운영 작업은 이미 완료(crontab·오프사이트 백업 검증까지 체크됨).
