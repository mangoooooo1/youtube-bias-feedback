// viewlens-ui.js — reusable UI components (return HTML strings)
// Depends on: viewlens-logo.js (markSVG), viewlens-data.js (VL)
// Exports (window): vlCard, vlSectionLabel, vlBadge, vlDeltaChip,
//   vlBarChart, vlDonut, vlDiversityMeter, vlMiniLine, vlReview
...
function vlDeltaChip({ value = 0, unit = '', invertColor = false } = {}) {
  ...
  const col = value === 0 ? 'var(--vl-ink-2)' : (good ? 'var(--vl-good)' : 'var(--vl-warn)');
...
function vlDiversityMeter({ value = 0, max = 3.17, lowAt = 1.9, highAt = 2.5 } = {}) {
  const zones = [
    { key: '편중', lo: 0,     hi: lowAt,  col: 'var(--vl-warn)',   desc: `0–${lowAt}` },
    { key: '보통', lo: lowAt, hi: highAt, col: 'var(--vl-ink-2)',  desc: `${lowAt}–${highAt}` },
    { key: '다양', lo: highAt,hi: max,    col: 'var(--vl-good)',   desc: `${highAt}+` },
  ];
  const ai = value < lowAt ? 0 : value < highAt ? 1 : 2;
  const z = zones[ai];
  const local = Math.max(0, Math.min(1, (value - z.lo) / (z.hi - z.lo)));
  const markP = ((ai + local) / 3) * 100;
  const zoneBars = zones.map((zn, i) =>
  ...
  const zoneLabels = zones.map((zn, i) =>
