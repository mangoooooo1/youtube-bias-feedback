...
describe("buildTodayCumulativePrompt — 오늘 하루 전체 프레이밍 (트렌드 문구 경계값 포함)", () => {
  ...
  it("다양성 증가/감소 문구는 방향과 무관하게 같은 강도(부드럽고 또렷하게)를 쓴다", () => {
    const increased = buildTodayCumulativePrompt(
      baseArgs({ videoCount: 5, entropy: 1.5, prevEntropy: 1.0 }),
    );
    const decreased = buildTodayCumulativePrompt(
...
describe("generateTodayFallbackReview — 오늘 하루 프레이밍", () => {
  it("promptVersion은 TODAY_PROMPT_VERSION이다", () => {
...
describe("generateReview — 실패 사유 분류 (failureReason 태깅)", () => {
  ...
  it("두 번째 인자로 promptVersion을 넘기면 기본값 대신 그 값이 찍힌다", async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        candidates: [
    ...
    const result = await generateReview("prompt", "custom-test-version");
    expect(result.promptVersion).toBe("custom-test-version");
  });
